...
writer = SummaryWriter("logs_finetune/GPT_finetune_"+str(datetime.now()).replace(" ","_"))
...
for e in range(MAX_EPOCHS):
    ...
    for idx in tqdm(range(EPOCHS)):
        episode_train_img, episode_train_label, episode_test_img, episode_test_label, episode_filenames = dataloader.get_batch_with_true_labels(phase='train', idx=idx)
        ...
        label_encoding = encoding_items["input_ids"][label_pos_global]
        ...
        encoding_label_final[:,-1] = label_encoding
    ...
    torch.save(model_vis.state_dict(), "saved_models/GPT_model_vis_updated_"+str(e)+"_"+timenow+".pth")
    torch.save(model_vis_ext.state_dict(), "saved_models/GPT_model_vis_updated_ext_"+str(e)+"_"+timenow+".pth")
pu.db